    ...
    for episode in tqdm(range(num_episodes)):
        ...
        while True:
            ...
            while not done and step < max_episode_length:
                ...
                #             [[0.8, 0.0], [-0.8, 0.0], [0, -0.8], [0, 0.8]]
                #         )
                if isinstance(env.unwrapped, envs.minigrid.traffic.TrafficEnv):
                    if step % 10 == 0:
                ...
                # TODO: implement optimal policies for environments that are clear

                actions[episode, step] = torch.tensor(action)
                masks[episode, step] = (
                    1 - done
                )  # The first 0 is where we no longer have data
