...
parser.add_argument(
    "--procs", type=int, default=1, help="number of processes (default: 1)"
)
parser.add_argument(
    "--frames",
    type=int,
...
parser.add_argument("--load-model", default=None, help="Directory of the model to load")
parser.add_argument(
    "--wandb",
    action="store_true",
    default=False,
...
parser.add_argument(
    "--epochs", type=int, default=1, help="number of epochs for PPO (default: 4)"
)
parser.add_argument(
    "--batch-size", type=int, default=100, help="batch size for PPO (default: 100)"
)
parser.add_argument(
    "--frames-per-proc",
    type=int,
...
parser.add_argument(
    "--hidden-size", type=int, default=128, help="hidden dimensions for actor-critic."
)
...
parser.add_argument(
    "--rm-update-algo", type=str, default="tdm", help="[tdm, naive, ibu, oracle, no_rm]"
)
...
in_model_dir = (
    None if args.load_model is None else utils.get_model_dir(args.load_model, "")
)
...
# Load model
detectormodel = getDetectorModel(
    envs[0], obs_space, args.rm_update_algo, use_mem_detector
)
...
# Otherwise, load detector model from a pretrained source if it exists
    ...
    try:
        pretrain_detector_status = utils.get_status(
            utils.get_model_dir(pretrained_model_name, storage_dir)
        )
...
algo = torch_ac.PPOAlgo(
    ...
    args.detector_lr,
    args.detector_recurrence,
    args.optim_eps,
    args.clip_eps,
    args.epochs,
    args.batch_size,
    preprocess_obss,
    rm_update_algo=args.rm_update_algo,
)
...
while num_frames < args.frames:
    ...
    if update % args.log_interval == 0:
        ...
        average_reward_per_step = utils.average_reward_per_step(
            logs["return_per_episode"], logs["num_frames_per_episode"]
        )
        average_discounted_return = utils.average_discounted_return(
            logs["return_per_episode"], logs["num_frames_per_episode"], args.discount
        )
        ...
        header += [
            "detector_loss_train",
            "detector_loss_test",
            "detector_top1_accuracy_train",
            "detector_top1_accuracy_test",
        ]
        data += [
            logs["detector_loss_train"],
            logs["detector_loss_test"],
            logs["detector_top1_accuracy_train"],
            logs["detector_top1_accuracy_test"],
        ]

        txt_logger.info(
            "U {} | F {:06} | FPS {:04.0f} | D {} | rR:μσmM {:.2f} {:.2f} {:.2f} {:.2f} | ARPS: {:.3f} | ADR: {:.3f} | F:μσmM {:.1f} {:.1f} {} {} | H {:.3f} | V {:.3f} | pL {:.3f} | vL {:.3f} | ∇ {:.3f} | dLtrain {:.3f} | dLtest {:.3f} | dtop1Acctrain {:.3f} | dtop1Acctest {:.3f}".format(
                *data
            )
        )
