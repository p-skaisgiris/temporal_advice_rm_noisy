...
# Load environment
env = make_env(
    args.env, args.rm_update_algo, args.seed, render_mode="human", screen_size=640
)
env.reset()
...
agent = utils.Agent(
    ...
    use_mem_detector=args.use_mem_detector,
    device=device,
)
print("Agent loaded\n")
...
for episode in range(args.episodes):
    ...
    while True:
        ...
        if agent.rm_belief is not None:
            env.unwrapped.mission = str(
                agent.pretty_print_belief()
            )  # viewer.add_overlay(const.GRID_TOPRIGHT, "RM State Belief", str(agent.pretty_print_belief()))
        ...
        if terminated | truncated or i == 50:
            print(
                "Episode %i --- Return: %.3f --- Num Steps: %d"
                % (episode + 1, returnn, i)
            )
