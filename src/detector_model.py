    ...
    if use_mem_detector:
        if rm_update_algo in ["naive", "ibu"]:
            if isinstance(env, MiniGridEnv):
                detectormodel = RecurrentMinigridDetectorModel(
                    obs_space, obs_space["events"]
                )
            elif isinstance(env, ColourEnv):
                detectormodel = RecurrentColourDetectorModel(
                    obs_space, obs_space["events"]
                )
            ...
            if isinstance(env, MiniGridEnv):
                ...
                detectormodel = HybridColourDetectorModel(
                    obs_space, obs_space["rm_state"], obs_space["events"]
                )
...
class RecurrentColourDetectorModel(BaseDetector):
    ...
    def __init__(self, obs_space, out_dim) -> None:
        ...
        self.decoder = nn.Sequential(
            nn.Linear(self.memory_size, 128), nn.ReLU(), nn.Linear(128, self.out_dim)
        )
...
class HybridColourDetectorModel(BaseDetector):
    ...
    def __init__(self, obs_space, n_rm_states, n_events) -> None:
        ...
        # image_embedding -> embedding
        self.memory_rnn1 = nn.GRUCell(
            self.image_embedding_size + self.n_events, self.semi_memory_size
        )
        self.memory_rnn2 = nn.GRUCell(self.semi_memory_size, self.semi_memory_size)
